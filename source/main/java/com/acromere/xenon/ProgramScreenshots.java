...
public abstract class ProgramScreenshots {
	...
	protected void openResource( URI uri ) throws InterruptedException, TimeoutException, ExecutionException {
		program.getResourceManager().openResource( uri );
	...
	private void startup( double scale ) throws InterruptedException, TimeoutException {
		try {
			...
			program = new Xenon();
			program.setProgramParameters( com.acromere.util.Parameters.parse( getProgramParameters() ) );
			program.init();
			Platform.startup( () -> {
				try {
