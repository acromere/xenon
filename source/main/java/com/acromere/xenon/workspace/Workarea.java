...
import com.acromere.zerra.event.FxEventWrapper;
	...
	private static class DropHandler implements DropListener {
		...
		public void handleDrop( DropEvent event ) {
			...
			if( sourceTool == null ) {
				...
				uris.forEach( u -> getProgram().getResourceManager().openResource( u, targetView, side ) );
			} else {
				if( mode == TransferMode.MOVE ) {
					...
					getProgram().getResourceManager().openResource( sourceTool.getResource(), targetView, side );
				}
			}
		}
