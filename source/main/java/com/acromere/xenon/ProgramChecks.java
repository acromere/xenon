...
import static com.acromere.xenon.Xenon.PROGRAM_RELEASE_PRIOR;
...
public class ProgramChecks implements Runnable {
	...
	private void checkForHiDpi() {
		if( isHiDpiCapable() && !isHiDpiEnabled() ) {
			...
			Runnable action = () -> getProgram().getResourceManager().openResource( URI.create( uriString ) );
	...
	private void checkForProgramUpdated() {
		if( program.isProgramUpdated() ) {
			...
			Runnable action = () -> program.getResourceManager().openResource( ProgramAboutType.URI );
